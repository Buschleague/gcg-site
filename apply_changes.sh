...
echo "==> Running a quick local build to validate TypeScript…"
npm run build || { echo "Build failed; fix any TS errors above."; exit 1; }

echo "==> Committing and pushing changes…"
git add -A
git commit -m "feat: lock hero; remove upload/slider; add brands (HBA, Sparkle Squad, OEH) with robust asset handling" || true
git push || true

echo "==> Done. GitHub Pages should redeploy via Actions on main."
